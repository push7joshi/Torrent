...
const int RequestSize = pow(2, 14);

//holds handshake information

using std::vector;

typedef struct handshake_msg {
    unsigned char protocol[20];
...
typedef struct {
    ...
    long long piece_length; //number of bytes in each piece
    long long length; //length of the file in bytes
    long long num_pieces; //number of pieces, computed based on above two values
    vector<bool> bit_field;
...
typedef struct {
  ...
  char seedFile[FILE_NAME_MAX]; // file to be uploaded/distributed -- just to be used by seeder.
...
typedef struct {
    int index; //which piece index
    int begin; //offset within piece
    char * block; //pointer to start of the data for a piece
} bt_piece_t;
...
//socket setup - create and return a socket
...
void sendInitialInterest(int socketFD);
    ...
    //Connect to the peer to start exchange after the handshake
void connectToPeer(bt_args_t bt_args, int currPeer, int am_choked, int am_interested);
